...
IS
    ...
    CONTADOR_EMAIL NUMBER;
    LAST_EMAIL VARCHAR2(50);
BEGIN
    FIRST_EMAIL := UPPER(CONCAT(SUBSTR(NOMBRE_USUARIO_F,1,2),CONCAT(SUBSTR(APELLIDO1_USUARIO_F,3,2),CONCAT('.',APELLIDO2_USUARIO_F))));
    SELECT COUNT(EMAIL) INTO CONTADOR_EMAIL
    FROM JMRP_USUARIO
    WHERE EMAIL LIKE FIRST_EMAIL || '@TESTSQL.CL';

    IF(CONTADOR_EMAIL = 0) THEN
        LAST_EMAIL := FIRST_EMAIL || '@TESTSQL.CL';
    ELSE
        LAST_EMAIL := FIRST_EMAIL || TO_CHAR(CONTADOR_EMAIL) || '@TESTSQL.CL';
    END IF;
    RETURN LAST_EMAIL;
END;
