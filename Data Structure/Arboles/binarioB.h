...
void eliminarNodo(ARBOL *nodo,int dato){
    ...
    if (nodo != NULL){
        //comprobar si es nodo interno
        if ((nodo->izq != NULL) && (nodo->der !=NULL)){
            // verificar que el nodo contenga el dato que se quiere eliminar
            if (nodo->dato == dato){
                if (nodo->izq < nodo->der){
                    //el nuevo dato debe tomar el valor del anterior
                    insertar(nodo->izq, nodo->izq->dato);
                }
            }
